...
// Fake response for PayFast webhooks and subscription API requests.
add_filter(
	'pre_http_request',
	function ( $result, $args, $url ) {
...
add_action(
	'woocommerce_thankyou_payfast',
	function ( $order_id ) {
		wp_remote_post(
			esc_url_raw( home_url( '/?wc-api=wc_gateway_payfast' ) ),
			array(
				'headers'   => array( 'Content-Type' => 'application/x-www-form-urlencoded' ),
				'body'      => ( new WebhookDataProvider( $order_id ) )->get_data(),
				'format'    => 'body',
				'sslverify' => false,
			)
		);
